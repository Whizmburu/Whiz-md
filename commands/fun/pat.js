async function handlePatCommand(msg, args, client, theme, botPrefix) {
    const chat = await msg.getChat();
    await chat.sendStateTyping();

    if (msg.mentionedIds.length === 0) {
        let replyMsgContent = theme.messages.interactiveCmd.noMention;
        replyMsgContent = replyMsgContent.replace('{prefix}', botPrefix).replace('{commandName}', 'pat');
    ...
    let replyMsgContent = theme.messages.interactiveCmd.pat;
